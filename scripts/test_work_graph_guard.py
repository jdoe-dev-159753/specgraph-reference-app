...
REQUIRED_PROTECTED_ASSETS = {
    ".github/workflows/project-v2-reconcile.yml",
...
class MainIntegrationTests(unittest.TestCase):
    ...
    def test_truncated_recursive_tree_fails_closed_without_files_listing(self):
        ...
        manifest = {
            ...
            "content": base64.b64encode(b"proof.yml\n").decode("ascii"),
        }

        def fake_api(path):
            self.assertNotIn("/files", path)
            if path.endswith("/pulls/309"):
                return pull_request
            if "/contents/scripts/ci/durable-workflows.txt?" in path:
                return manifest
            if "/git/trees/" in path:
    ...
    def test_every_open_main_pr_gets_full_tree_and_guard_source_audit(self):
        ...
        def fake_api(path):
            ...
            if "/git/trees/" in path:
                return {"truncated": False, "tree": [
                    {"type": "blob", "path": ".github/workflows/proof.yml"}
                ]}
            return payload
...
class DurableWorkflowTests(unittest.TestCase):
    ...
    def test_exact_canonical_inventory_is_accepted(self):
        workflows = {
            "application-ci.yml": self.valid_workflow("application-ci"),
            "r4-acceptance-ci.yml": self.valid_workflow("r4-acceptance-ci"),
        }
    ...
    def test_status_writers_and_projects_token_are_reserved(self):
        ...
        for candidate in (status_spoof, checks_spoof, flow_spoof, token_spoof):
            with self.subTest(candidate=candidate):
                self.assertTrue(guard.durable_workflow_policy_violations("proof.yml", candidate))
    ...
    def test_runner_attestation_avoids_secret_and_process_disclosure(self):
        root = Path(__file__).resolve().parents[1]
        text = (root / guard.WORKFLOW_DIR / "runner-host-attestation.yml").read_text(encoding="utf-8")
        self.assertIn('(\"agentId\", \"agentName\", \"workFolder\")', text)
        self.assertIn("pgrep -x Runner.Listener", text)
        self.assertIn("stat -c 'docker-socket=", text)
        self.assertNotIn("/cmdline", text)
        self.assertNotIn("printenv", text)
        self.assertNotIn("cat $runner_root/.credentials", text)
