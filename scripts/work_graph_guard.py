...
ONE_SHOT_WORKFLOW = re.compile(
    r"(?<![A-Za-z0-9])(?:(?:prs?|pull[^A-Za-z0-9]+requests?|issues?|discover(?:y|ies)|stor(?:y|ies)|fix(?:es)?)(?:no|number|id)?(?![A-Za-z0-9])[^\n]*?(?:0x[0-9a-f]+|\d+)"
    r"|(?:0x[0-9a-f]+|\d+)[^\n]*?(?<![A-Za-z0-9])(?:prs?|pull[^A-Za-z0-9]+requests?|issues?|discover(?:y|ies)|stor(?:y|ies)|fix(?:es)?))(?![A-Za-z0-9])",
    re.IGNORECASE,
)
    ...
    while index < len(text):
        ...
        if quote:
            ...
            if char == "\n":
                ...
                escaped = False
        elif char in {"'", '"'} and re.search(r"(?:^\s*(?:-\s+)?|:\s+)$", text[text.rfind("\n", 0, index) + 1:index]):
            quote = char
            result.append(" ")
...
def durable_workflow_policy_violations(filename: str, text: str) -> list[str]:
    ...
    surfaces = [line for line in re.sub(r"\\\r?\n[ \t]*", "", text).splitlines()
                if line != CANONICAL_QUEUE_GROUP and not line.startswith("    if: ${{ ")]
    for index, line in enumerate(lines):
        if re.search(r":\s*>(?:[+-]?[1-9]?|[1-9][+-])\s*(?:#.*)?$", line):
            surfaces.append(" ".join(entry.strip() for entry in _indented_block(lines, index, len(line) - len(line.lstrip(" ")))))
