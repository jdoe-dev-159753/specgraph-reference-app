...
/** Creates bounded source-shaped stories without deriving analysis or AML conclusions. */
@Service
final class DemoScenarioService implements DemoScenarioUseCase {
    ...
    DemoScenarioService(DemoScenarioPersistencePort persistence) { this.persistence = persistence; }
    /** Generates one deterministic six-activity story and retains replay provenance. */
    @Override
    public ScenarioResult generate(long seed, ScenarioFamily family) {
        ...
        persistence.save(new ScenarioData(seed, family, GENERATOR_ID, anchor, snapshot));
        return new ScenarioResult(customerId, Long.toString(seed), family, GENERATOR_ID, 6,
                snapshot.riskEvidence().size());
    }
    /** Builds one coherent family-specific source activity from the seeded random stream. */
    ...
    /** Derives only synthetic source risk evidence appropriate to the selected story family. */
    private static List<RiskEvidence> risks(String identity, ScenarioFamily family, List<Activity> activities) {
        if (family == ScenarioFamily.ORDINARY_LOCAL) return List.of();
        List<RiskEvidence> risks = new ArrayList<>();
        Activity crossBorder = activities.stream().filter(activity -> activity.details() instanceof Activity.PaymentDetails details
                && !details.receiverBankCountry().equals("CH")).findFirst().orElseThrow();
    ...
    private static String foreignCurrency(long seed) { return Math.floorMod(seed, 2) == 0 ? "EUR" : "USD"; }
    private static UUID rule(String suffix) { return UUID.fromString("10000000-0000-0000-0000-00000000000" + suffix); }
    private static UUID stableId(String value) { return UUID.nameUUIDFromBytes(value.getBytes(StandardCharsets.UTF_8)); }
    private static BigDecimal money(int cents) { return BigDecimal.valueOf(cents, 2); }
    private static int seedPart(SplittableRandom random) { return random.nextInt(100_000, 999_999); }
}
