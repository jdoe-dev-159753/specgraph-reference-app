...
The retained R1-R4 image crops come from Playwright artifacts. The build also creates pixel-preserving R5 crops from `docs/reviewer/screenshots/R5_lmstudio_ensemble_customer_444.png`; the original PNG and `docs/reviewer/screenshot-manifest.md` remain the provenance authority. If the live demonstration is unavailable, those screenshots form the mandatory fallback. A recorded walkthrough is optional. The conceptual WorkGraph, delivery, milestone and burn-up diagrams explain management views without requiring a private GitHub Project capture.
