...
The runtime projects an 8,192-token context window, a 512-token maximum output and a 256-token transport/tokenizer margin by default. Configure the loaded Ministral instance to the same 8,192-token context before the demonstration. The complete customer `444` evidence envelope is conservatively estimated at 4,163 total tokens, so this leaves comfortable headroom without approaching the model's declared maximum. `SPECGRAPH_LOCAL_CONTEXT_WINDOW_TOKENS`, `SPECGRAPH_LOCAL_MAX_OUTPUT_TOKENS` and `SPECGRAPH_LOCAL_TRANSPORT_MARGIN_TOKENS` can override those limits when the loaded model instance is configured differently. The launcher validates the response's system, user and schema estimates, recomposes their input subtotal, then requires `estimated input + transport margin + maximum output = estimated total <= context window`. Its successful manifest uses the returned model, prompt, runtime and estimator values rather than configured labels. Status reports configured limits and LM Studio reachability but honestly marks response-only fields `not-evaluated-by-status` because it does not create another analysis.
...
The four scenarios were deliberately written as an ordinal story: ordinary `222` < mixed reviewer seed `111` < growing cross-border `333` < dense mixed-risk `444`. The Bayesian detector follows that order with well-separated deterministic values: `0.028`, `0.420`, `0.594`, `0.901`. Issue [#441](https://github.com/jdoe-dev-159753/specgraph-reference-app/issues/441) replaced the saturated fuzzy surface with overlapping memberships, explicit small-sample shrinkage and fixed graded consequents. Its resulting values — `0.050`, `0.491`, `0.619`, `0.717` — now follow the same crafted scenario order while remaining a heuristic rather than a calibrated probability. The Random Forest produces `0.000`, `0.968`, `0.484`, `0.710`; its vote share demonstrates a packaged learned adapter, but the tiny synthetic training generator does not make that non-ordinal number a better oracle.

R5 therefore retains all three artifacts without averaging them. The fuzzy and Bayesian artifacts both mirror the crafted scenario progression; Bayesian has the wider numeric separation, while fuzzy makes the rule-based intent directly inspectable. This is an internal consistency observation, not an out-of-sample accuracy or calibration claim.
