...
The repository-owned `r4-variant-*` scripts invoke `compose.r4.yaml` with a distinct Compose project per variant, so application network and PostgreSQL state remain isolated. `r4-gallery-*` exposes the deterministic baseline and optional external variant as parallel R4 configurations, not additional delivery rings. Starting the gallery without `OPENAI_API_KEY` explicitly stops any previously launched external project before attempting baseline startup, so a later baseline failure cannot defeat the credential opt-out. The manifest records port, ring, Compose project, Stage-3 backend, expected external-transmission mode and persistence isolation. The Linux Docker host is the canonical execution environment for this gallery.
