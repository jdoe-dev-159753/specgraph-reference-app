...
| `RiskSignalDetectorPort` | derive separately identified non-source risk signals from a `CustomerSnapshot` | typed ordered `specgraph.analysis.detectors` selection resolves `NO_OP`, Bayesian, fuzzy or packaged Random Forest as one leaf or a bounded Composite; legacy detector profiles are compatibility aliases only when typed selection is absent |
...
AnalysisHttpAdapter
  ...
  -> AnalysisModelPort
       [AnalysisBackendFactory: deterministic default | explicit OpenAI | private LM Studio]
       -> AnalysisModelOutput
...
This composition means RAG is one context-supply stage, not the complete AI architecture. Typed Stage-1 selection can resolve Bayesian, fuzzy or packaged Random Forest individually, or compose concrete leaves through `CompositeRiskSignalDetector`; further graph leaves may register later without changing `AnalysisService`. The Composite preserves child artifacts and ordering but performs no score calibration or fusion; #254 owns any later calibrated ensemble evidence. Deterministic, OpenAI and LM Studio Stage-3 strategies are selected independently through `specgraph.analysis.backend`. The local leaf reuses the bounded evidence, explicit JSON Schema and grounding validation of the OpenAI leaf, but its endpoint must use a loopback or private/link-local IP literal; hostnames are rejected so the validated address cannot be replaced by a later DNS answer. Its provenance records `externalTransmission=false`. All adapters translate library/provider-native results into existing application-owned sealed record variants. Neither substitution changes the application use case or grants generated output authority over source `risk_assessments`.

`AnalysisBackendId` is the application-owned Stage-3 selection contract. Spring Boot property, command-line and environment precedence converge on `specgraph.analysis.backend`, with `SPECGRAPH_ANALYSIS_BACKEND` as its environment projection and `deterministic` as the safe default. The derived `spring.ai.model.chat` activation must match that authoritative selection; any higher-precedence mismatch fails startup rather than becoming a second selector. Provider settings such as `OPENAI_API_KEY`/`OPENAI_MODEL` or the local base URL/model/optional token configure one leaf but do not select it. `AnalysisBackendFactory` keeps adapter suppliers lazy, publishes one selected `@Primary AnalysisModelPort`, and fails startup for unknown or unavailable selections instead of silently falling back. Compose projects receive provider tokens only through backend-gated launcher projections so ambient credentials do not enter unrelated variants.
...
- R4 external variant: host `8087` -> the same current R4 artifact with explicit `backend=openai`, only when a credential is deliberately supplied.

The repository-owned `r4-variant-*` scripts invoke `compose.r4.yaml` with a distinct Compose project per variant, so application network and PostgreSQL state remain isolated. `r4-gallery-*` exposes the deterministic baseline and optional external variant as parallel R4 configurations, not additional delivery rings. The Compose definition does not interpolate the conventional ambient `OPENAI_API_KEY`; it accepts only the launcher-owned `SPECGRAPH_OPENAI_API_KEY` projection. The launcher clears the ambient key and populates that projection only for the OpenAI variant, so raw deterministic Compose launches cannot inherit an operator credential either. Starting the gallery without a key explicitly stops any previously launched external project before attempting baseline startup, so a later baseline failure cannot defeat the credential opt-out. Gallery shutdown attempts both projects and returns nonzero if either teardown fails. The manifest records port, ring, Compose project, Stage-3 backend, expected external-transmission mode and persistence isolation. The Linux Docker host is the canonical execution environment for this gallery.
...
- how implemented Bayesian/fuzzy detectors and typed deterministic/OpenAI/LM Studio Stage-3 selection substitute independently behind project-owned ports;
- why credentials configure a provider leaf but never select it, why local endpoints are constrained to private networking, and why unknown or unavailable Stage-3 selections fail closed;
