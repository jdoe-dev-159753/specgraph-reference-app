...
Stage-3 selection verification treats `specgraph.analysis.backend` as the single authoritative dimension. It proves the no-credential deterministic default, explicit OpenAI selection without activating unrelated model families, rejection of a mismatched `spring.ai.model.chat` override, configuration-binding rejection of unknown values, and explicit `local` selection of only the LM Studio adapter. The local proof uses a deterministic loopback OpenAI-compatible server to verify the configured model, shared grounded prompt and bounded evidence, native JSON Schema request, response-side schema validation, exact evidence references, local provenance and `externalTransmission=false`; missing, public and hostname-based local endpoints fail closed. Provider credentials and model settings must not select a backend implicitly. Compose/script evidence additionally checks that parallel R4 variants use distinct Compose projects, ports and PostgreSQL state, that unused OpenAI/local credentials do not enter another backend's container, and that each manifest reports the selected backend and external-transmission expectation truthfully.
