...
Run the optional local Stage-3 variant from `watch-infra-01` with LM Studio listening on the Windows host. Replace the placeholder with the Windows LAN address; enable LM Studio authentication/network listening and restrict the Windows firewall rule to the VPS address. `localhost:1234` is valid only when the application itself runs on Windows.
