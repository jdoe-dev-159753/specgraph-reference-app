...
`./scripts/r4-gallery-up.sh` starts the deterministic `8084` baseline and adds the `8087` OpenAI variant only when `OPENAI_API_KEY` is present. The baseline launcher explicitly clears that credential before invoking Compose, so a key supplied for the external variant never reaches the deterministic container. Without a key the gallery first stops any previously launched external project, so the opt-out remains effective even if baseline startup later fails. Distinct Compose project names isolate each variant's network and PostgreSQL state. Inspect the declared variants with `./scripts/r4-gallery-manifest.sh`, and stop them with `./scripts/r4-gallery-down.sh`; shutdown attempts both variants and reports failure if either teardown fails.
